...
export const registrationsRouter = router({
  ...
  applyCredit: staffProcedure
    .input(z.object({
      parentCreditId: z.string().uuid(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      return ctx.prisma.$transaction(async (tx) => {
        // Lock credit row to prevent concurrent double-spend
        const [credit] = await tx.$queryRawUnsafe<Array<{
          id: string;
          amount_remaining: number;
          expires_at: Date | null;
        }>>(
          `SELECT id, amount_remaining, expires_at FROM parent_credits WHERE id = $1 FOR UPDATE`,
          input.parentCreditId,
        );
        ...
        const newRemaining = Number(credit.amount_remaining) - input.amount;

        await tx.parentCredit.update({
          where: { id: input.parentCreditId },
          data: { amountRemaining: newRemaining },
        });

        await tx.creditApplication.create({
          data: {
            parentCreditId: input.parentCreditId,
        ...
        // Update invoice paidAmount if an invoice is specified
        if (input.invoiceId) {
          ...
          if (invoice) {
            const newPaid = toNum(invoice.paidAmount) + input.amount;
            const total = toNum(invoice.totalAmount);
            await tx.invoice.update({
              where: { id: input.invoiceId },
              data: {
                paidAmount: newPaid,
                status: newPaid >= total ? 'PAID' : undefined,
              },
            });
          }
        }
